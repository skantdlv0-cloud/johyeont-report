  ...
  function syncFieldDefs(list, opts) {
    ...
    if (!rows.length) {
      /* 빈 목록으로 서버를 비우는 것은 칸 관리에서 직접 지웠을 때만 한다.
         예전에 백업 불러오기가 빈 목록을 보내 서버의 칸 정의가 통째로 날아갔고,
         보여 줄 칸이 없어져서 반 현황이 사라진 것처럼 보였다.
         허락 없이 온 빈 목록은 지우지 않고, 서버에 남은 개수만 세어 돌려준다. */
      if (!opts.allowClear) {
        return c.from('field_defs').select('id')
          .then(check)
          .then(function (res) {
            var n = (res.data || []).length;
            return { skipped: true, refusedClear: n > 0, serverCount: n };
          });
      }
