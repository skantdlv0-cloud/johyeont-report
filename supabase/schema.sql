...
--    그것은 아래 RLS 가 모든 테이블에 켜져 있을 때만 성립한다.
--    RLS 를 끄면 키를 아는 누구나 전체 데이터를 읽는다.
--    secret key 는 절대 브라우저·저장소에 넣지 않는다.
-- ============================================================
...
-- ------------------------------------------------------------
--  1. students — 학생 명단
--     extra 는 '학생 관리 탭'에서 늘리는 칸의 값이 들어가는 자리다.
--     칸을 추가해도 테이블 구조를 바꿀 필요가 없다.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
--  2. field_defs — 학생 관리 탭에서 늘리는 칸의 '정의'
--     화면에서 "+ 칸 추가" 를 누르면 여기에 한 줄이 생긴다.
-- ------------------------------------------------------------
create table if not exists public.field_defs (
  id          uuid primary key default gen_random_uuid(),
  key         text not null,                       -- extra 안에서 쓰는 이름
...
create table if not exists public.sent (
  ...
  week_start  date not null,
  student_id  uuid not null references public.students(id) on delete cascade,
  via         text not null default 'copy',        -- sms | copy | share
